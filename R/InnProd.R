...
#' Est$ci
InnProd <- function(X1, y1, X2, y2, G, A = NULL, model = c("linear","logistic","logistic_alter"), intercept = TRUE, beta.init1 = NULL, beta.init2 = NULL, lambda = NULL, mu = NULL, prob.filter = 0.05, rescale = 1.1, alpha = 0.05){
  ...
  Est1 <- SIHR::LF(X1, y1, loading.mat = loading2, model = model, intercept = intercept, intercept.loading = FALSE, beta.init = beta.init1, lambda = lambda, mu = mu, prob.filter = prob.filter, rescale = rescale, alpha = alpha, verbose = TRUE)
  Est2 <- SIHR::LF(X2, y2, loading.mat = loading1, model = model, intercept = intercept, intercept.loading = FALSE, beta.init = beta.init2, lambda = lambda, mu = mu, prob.filter = prob.filter, rescale = rescale, alpha = alpha, verbose = TRUE)

  est.plugin <- as.numeric(t(beta.init1[G]) %*% A %*% beta.init2[G])

  est.debias <- est.plugin + Est1$correction + Est2$correction ### returned by LF and multiplied with the norm

  if(nullA){
    V.A = sum((as.vector(t(beta.init1[G])%*%t(X[,G,drop=F])%*%X[,G,drop=F]%*%beta.init2[G]) -
                 as.numeric(t(beta.init1[G]) %*% A %*% beta.init2[G]))^2) /(nrow(X))^2
    se <- sqrt(Est1$se.vec^2 + Est2$se.vec^2 + V.A)

  }else{
    se <- sqrt(Est1$se.vec^2 + Est2$se.vec^2)
  }
