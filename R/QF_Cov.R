...
#'
#' @description Computes the bias-corrected estimator of \eqn{\beta_G^{\top}\Sigma_{G,G}\beta_G} for the following high-dimensional
#' linear regression model \deqn{y=X\beta+\epsilon}
#' as \deqn{\widehat{\beta_G^{\top}\Sigma_{G,G}\beta_G}=\widehat{\beta}_{G}^{\top} \widehat{\Sigma}_{G, G} \widehat{\beta}_{G}+\frac{2}{n} \widehat{u}^{\top} X^{\top}(y-X \widehat{\beta})}
#' where \eqn{\Sigma} is the population design covariance matrix and \eqn{G\subset{1,2,...,p}}
#'
...
#' @param init.Lasso initial LASSO estimator \eqn{\widehat{\beta}} (default = \code{NULL})
#' @param tau.vec Vector of enlargement factors for asymptotic variance of the estimator \eqn{\widehat{\beta_G^{\top}\Sigma_{G,G}\beta_G}} to handle super-efficiency (default = \code{NULL})
#' @param lambda Tuning parameter \eqn{\lambda} used in construction of initial LASSO estimator \eqn{\widehat{\beta}} if \code{init.Lasso = NULL} (default = \code{NULL})
#' @param intercept Should intercept(s) be fitted (default = \code{FALSE})
#' @param mu Tuning parameter \eqn{\mu} in construction of projection direction \eqn{\widehat{u}} (default = \code{NULL})
...
#' that gives convergence of the optimization problem for constructing the projection direction \eqn{\widehat{u}} (default = 1.5)
#' @param maxiter Maximum number of steps along which \code{mu} is increased/decreased to obtain the smallest \code{mu}
#' that gives convergence of the optimization problem for constructing the projection direction \eqn{\widehat{u}} (default = 6)
#'
#' @return
#' \item{prop.est}{The bias-corrected estimator \eqn{\widehat{\beta_G^{\top}\Sigma_{G,G}\beta_G}}}
#' \item{sigma}{Estimate of the variance of error term \eqn{\epsilon} in the linear regression model}
#' \item{se}{Standard error of \eqn{\widehat{\beta_G^{\top}\Sigma_{G,G}\beta_G}}}
#' \item{plug.in}{Plug-in LASSO estimator \eqn{\widehat{\beta}_{G}^{\top} \widehat{\Sigma}_{G, G} \widehat{\beta}_{G}}}
#' @export
#'
...
#' QF_Cov(X = X, y = y, test.set=c(30:50))
#'
#' @references
#'
#' \insertRef{grouplin}{FIHR}
QF_Cov <- function(X, y, test.set, init.Lasso = NULL, tau.vec = NULL,
